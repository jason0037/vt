...
def login_in
return render :text=>params[:return_url]
     #params[:visitor]
     @v = Ecstore::Visitor.where(:visitor_name=>params[:visitor][:visitor_name],:visitor_password=>params[:visitor][:visitor_password])
...
def register_user
  ...
  visitor=Ecstore::Visitor.where(:visitor_name=>params[:visitor][:visitor_name])
len =visitor.length
   if len==0

  @visitor=Ecstore::Visitor.new do |v|
   v.visitor_name=params[:visitor][:visitor_name]
...
def istrue
   ...
   visitor=Ecstore::Visitor.where(:visitor_name=>params[:visitor_name])
  len = visitor.length
   if len==0
    return "0"
    else
